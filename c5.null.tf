...
# Connection Block for Provisioners to connect to Azure VM Instance
  connection {
    ...
    private_key = file("${path.module}/ssh-key/terraformazure.pem")
  }
...
## File Provisioner: Copies the terraform-key.pem file to /tmp/terraform-key.pem
  provisioner "file" {
    source = "ssh-key/terraformazure.pem"
    destination = "/tmp/terraformazure.pem"
  }
## Remote Exec Provisioner: Using remote-exec provisioner fix the private key permissions on Bastion Host
  provisioner "remote-exec" {
    inline = [
      "sudo chmod 400 /tmp/terraformazure.pem"
